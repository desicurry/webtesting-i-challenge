...
const item1 = {
  ...
  describe("enhancer module", () => {
    describe("repair function", () => {
      test("can return a new item with 100 durability", () => {
    ...
    describe("fail function", () => {
      test("can return a new item with -5 durability if enhancement is < 15", () => {
        const expectedDurability = 45;
        const actualOutput = enhancer.fail(item2);
        expect(actualOutput.durability).toBe(expectedDurability);
      });
      test("can return a new item with -10 durability if enhancement is >= 15", () => {
        const expectedDurability = 60;
        const actualOutput = enhancer.fail(item3);
        expect(actualOutput.durability).toBe(expectedDurability);
      });
      test("can return a new item with -1 enhancement if enhancement is >= 16", () => {
    ...
    describe("succeed function", () => {
      test("can return a new item with + 1 enhancement", () => {
        const expectedEnhancement = 8;
        const actualOutput = enhancer.succeed(item5);
        expect(actualOutput.enhancement).toBe(expectedEnhancement);
      });
      test("can return a new item with unchanged enhancement if already 20", () => {
